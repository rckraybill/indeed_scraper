...
def insert_record_to_es(es_instance, index, document):

    es_instance.index(index = index, doc_type='type', body=document)
...
def grab_job_links(soup):
    ...
    Parameters:
        soup: the soup object corresponding to a search result page
                e.g. https://www.indeed.com/jobs?q=data+scientist&l=Chicago&start=20
...
def get_posting(url):
    ...
    company_and_location = soup.find(name='div', attrs={'class': "jobsearch-DesktopStickyContainer"}).get_text()

    return title, posting.lower(), company_and_location
...
def get_data(query, num_pages, location='Chicago'):
    """
    ...
    if isinstance(urls, list):
        ...
        for i, url in enumerate(urls):
            try:
                title, posting, company_and_location = get_posting(url)
                # postings_dict[i] = {}

                document = {'title':title, 'company_and_location': company_and_location, 'posting':posting, 'url':url}
                
                postings_list.append(document)
                
                index_name = 'index_{}'.format(query.replace('+','_')) 

                insert_record_to_es(es, index_name , document)
...
if __name__ == "__main__":
    queries = ["dev ops engineer", "python engineer", "data engineer", "aws engineer"]
    ...
    get_data(query, num_pages, location='Chicago')
